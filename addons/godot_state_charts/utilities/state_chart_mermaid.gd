...
func _ready() -> void:
	...
	# iterate and record all the state nodes first. if iterate transitions at same time, the target of transition may have not seen
...
func generate_states_string_dfs(node, depth: int = 0) -> void:
	...
	# Get the count of this node name to handle duplicates
	var count := get_count(name_count, node.name)
	# Create a unique name by appending count if needed
	var unique_name: String = node.name + ('' if count == 0 else str(count))
	# Append "(Parallel)" for ParallelState nodes
	...
	if node is CompoundState or node is ParallelState:
		...
		# Recursively process all children of this compound/parallel state
		for child in node.get_children():
			generate_states_string_dfs(child, depth + 1)
		# If it's a CompoundState and has an initial state, add transition from [*] to initial state
		...
		# Recursively process children of this state
		for child in node.get_children():
			generate_states_string_dfs(child, depth + 1)
